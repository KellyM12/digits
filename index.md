...
Digits is an application that can help you store your contacts information:

- Can create an account and add/edit contacts
- Can add timestamped notes to each individual contact

The goal is to help easily access your contacts and any relevant information associated with them.
...
Second, go to [s](https://github.com/KellyM12/digits), and click the "Use this template" button. Complete the dialog box to create a new repository that you own that is initialized with this template's files.
...
Fifth, create a `.env` file from the `sample.env`. Edit the line `DATABASE_URL="postgresql://<username>:<password>@localhost:5432/mydb?schema=public"` to match your setup. Replace `mydb` with the PostgreSQL database that you created in the first step (in the example for this step the database is `digits`). replace `<username>` and `<password>` with a username and password you created for this db. If you did not create a user for this database, you can use the `postgress` user with the password you set when you installed postgress. Note: this is not a recommdened practice since the `postgres` user is an admin with full access to postgres. 
...
Once the libraries are installed and the database seeded, you can run the application by invoking the "dev" script in the [package.json file](https://github.com/KellyM12/digits/blob/main/package.json):
...
app/
  ...
  components/
    ...
    AddContactForm.tsx # The Form for adding Note.
    ...
    NoteItem.tsx # Within individual Contact, the Note section.
...
The Contact and Note models are defined in [prisma/schema.prisma](https://github.com/KellyM12/digits/blob/main/prisma/schema.prisma).

The tables are initialized in [prisma/seed.ts](https://github.com/KellyM12/digits/blob/main/prisma/seed.ts) using the command `npx prisma db seed`.
...
body {
  ...
  padding: 0;
  margin: 0;
  background: fixed center url("../../public/images/sky.jpg");
...
When the database is seeded, a settings file (such as [config/settings.development.json](https://github.com/KellyM12/digits/blob/main/config/settings.development.json)) is used to create contacts and notes in the PostgreSQL database. That will lead to a default accounts being created.
...
The [config](https://github.com/KellyM12/digits/tree/main/config) directory is intended to hold settings files. The repository contains one file: [config/settings.development.json](https://github.com/KellyM12/digits/blob/main/config/settings.development.json).

The [.gitignore](https://github.com/KellyM12/digits/blob/main/.gitignore) file prevents a file named settings.production.json from being committed to the repository. So, if you are deploying the application, you can put settings in a file named settings.production.json and it will not be committed.

### Quality Assurance

#### ESLint

The application includes a [.eslintrc.json](https://github.com/KellyM12/digits/blob/main/.eslintrc.json) file to define the coding style adhered to in this application. You can invoke ESLint from the command line as follows:
